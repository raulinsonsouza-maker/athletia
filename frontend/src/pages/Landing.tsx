...
export default function Landing() {
  ...
  const iniciarOnboarding = useCallback(() => {
    ...
    // Se já estamos no onboarding (step > 0), não fazer nada
    if (step > 0) {
      console.log('[Landing] Já está no onboarding, ignorando chamada')
      return
    }
  ...
  const handleDiscardDraft = useCallback(() => {
    ...
    setTimeout(() => {
      console.log('[Landing] Iniciando onboarding após descartar draft')
      setStep(1)
    }, 150)
  }, [clearDraft])
              ...
              <div className="space-y-10 text-center">
                ...
                <div className="grid md:grid-cols-2 gap-6 md:gap-8">
                  {[
                    ...
                    <div key={index} className="flex items-start gap-4 group bg-dark-lighter/50 rounded-xl p-6 border border-grey/20 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10 transition-all">
      ...
      {showRestoreModal && draftStep !== null && (
        <OnboardingRestoreModal
          ...
          onClose={() => {
            ...
            setShowRestoreModal(false)
            // Se fechar sem escolher, descartar o draft e iniciar do zero
            clearDraft()
            setTimeout(() => {
              setStep(1)
            }, 100)
