      ...
      <div className="p-5 space-y-5 pb-32">
        {exercicioEmFoco && (
          <section className="bg-white/5 border border-white/10 rounded-3xl p-5 space-y-4 backdrop-blur">
            ...
            <div className="rounded-3xl overflow-hidden bg-dark-lighter border border-white/5">
              {exercicioEmFoco.gifUrl ? (
                <img src={exercicioEmFoco.gifUrl} alt={`Execução de ${exercicioEmFoco.nome}`} className="w-full h-52 object-cover" />
              ) : (
        ...
        {!loading && plano && plano.blocos.length === 0 && (
          <div className="bg-dark-lighter rounded-3xl p-6 text-center">
            ...
            <button onClick={() => navigate('/treino-rapido')} className="bg-primary text-dark font-semibold px-4 py-2 rounded-full">
              Criar treino rápido
            </button>
          </div>
        )}
        ...
        {!loading &&
          ...
          plano.blocos.map((bloco, index) => (
            <BlocoTreino
              ...
              onToggleExercicio={(treinoId, exercicio, marcado) => handleToggleExercicio(treinoId, exercicio, marcado)}
              emFocoId={exercicioEmFoco?.id}
              onSelectExercicio={(ex) => setExercicioAtivo(ex)}
            />
          ))}
        ...
        {blocoAtivo && (
          <div className="bg-white/5 border border-white/10 rounded-3xl p-4 space-y-2">
            <div className="flex items-center justify-between text-sm text-white/70">
              <span>
                {blocoAtivo.exercicios.filter((ex) => statusExercicios[ex.id]).length}/{blocoAtivo.totalExercicios} exercícios concluídos
              </span>
...
const BlocoTreino = ({
  ...
  onToggleExercicio: (treinoId: string, exercicio: PlanoAtualExercicio, marcado: boolean) => void
  emFocoId?: string
  onSelectExercicio: (exercicio: PlanoAtualExercicio) => void
}) => (
  <div className="bg-dark-lighter rounded-3xl overflow-hidden border border-white/5">
    <button className="w-full flex items-center justify-between px-5 py-4" onClick={onToggle}>
      ...
      <span className="text-light-muted">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">
          {aberto ? <path strokeLinecap="round" strokeLinejoin="round" d="M18 15l-6-6-6 6" /> : <path strokeLinecap="round" strokeLinejoin="round" d="M6 9l6 6 6-6" />}
        </svg>
      </span>
    </button>
    {aberto && (
          ...
          >
            <button
              onClick={(event) => {
                ...
                onToggleExercicio(bloco.id, exercicio, !statusExercicios[exercicio.id])
              }}
