...
interface CadastroFormSectionProps {
  onSubmit: (formData: {
    nomeCompleto: string
    telefone: string
    email: string
...
export default function CadastroFormSection({ onSubmit, loading = false, error, onScrollIntoView = false }: CadastroFormSectionProps) {
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className={`bg-dark-lighter/70 backdrop-blur-xl rounded-2xl md:rounded-3xl border-2 border-primary/30 p-6 md:p-8 shadow-2xl shadow-primary/20 transition-all duration-1000 delay-300 ${
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="space-y-5 md:space-y-6">
            ...
            {/* Card Informativo */}
            <div className="rounded-xl bg-gradient-to-r from-primary/20 to-primary/10 border-2 border-primary/30 p-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-light-muted">
                    Seus dados estão protegidos. Após o cadastro, você será redirecionado para escolher seu plano.
                  </p>
                </div>
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full btn-primary py-5 text-lg md:text-xl font-bold shadow-2xl shadow-primary/50 hover:scale-105 active:scale-95 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
            >
              {loading ? 'Criando conta...' : 'Finalizar Cadastro e Continuar'}
            </button>
          </form>
        </div>
      </div>
